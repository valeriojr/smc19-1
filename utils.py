...
def run(suspect=900, sick=240, hospital=48, icu=12, seed=42):
    ...
    for i, city in enumerate(cities):
        create_dummy_profiles(int(data["suspect"]*distribution[i]), suspect=True, city=city[1])
        create_dummy_profiles(int(data["sick"]*distribution[i]), result="PO", city=city[1])
        create_dummy_profiles(int(data["hospital"]*distribution[i]), result="PO", city=city[1])
        create_dummy_profiles(int(data["icu"]*distribution[i]), result="PO", city=city[1])
